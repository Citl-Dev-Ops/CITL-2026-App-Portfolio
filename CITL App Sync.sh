...
LINUX_EXE="$HERE/1-CITL-SYNC/linux/CITL App Sync/CITL App Sync"
if [ -x "$LINUX_EXE" ]; then
    nohup "$LINUX_EXE" &>/dev/null &
    echo "[ OK ] Launched (Linux binary): CITL App Sync"
    exit 0
fi
...
# Find the script
for script_path in     "$HOME/Desktop/CITL/factbook-assistant/citl_app_sync.py"     "$HOME/Documents/CITL/factbook-assistant/citl_app_sync.py"     "$HOME/CITL/factbook-assistant/citl_app_sync.py"     "$HERE/factbook-assistant/citl_app_sync.py"; do
...
Please enter your password if prompted." 2>/dev/null &
        ...
        echo "[ OK ] Launched: CITL App Sync"
        exit 0
    fi
done

echo "[ERROR] CITL App Sync not found. Run START_CITL_UBUNTU.sh to install first."
if command -v zenity &>/dev/null; then
    zenity --error --text="CITL App Sync not found.
