...
Write-Host "  CITL Document Composer" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan

# ---- Locate repo root (two levels up from scripts/windows) ------------------
$Repo   = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path
$Script = Join-Path $Repo "factbook-assistant\citl_doc_composer.py"
$Exe    = Join-Path $Repo "dist\CITL Document Composer\CITL Document Composer.exe"
Write-OK "Repo   : $Repo"
...
if ($repoDrive -ne "C:" -or $Portable) {
    $CacheBase = Join-Path $env:LOCALAPPDATA "CITL_USB_CACHE\citl_doc_composer"
    $env:CITL_PORTABLE = "1"
} else {
    $CacheBase = $Repo
}
...
if (-not (Test-Path $VenvPy)) {
    ...
    if (Test-Path $req) {
        ...
        & $VenvPy -m pip install requests psutil python-docx pillow --quiet
    }
...
$log = Join-Path $Repo "factbook-assistant\citl_doc_composer_crash.log"
if ($proc.HasExited) {
    $ec = $proc.ExitCode
