...
function Ensure-PythonDeps {
  ...
  if ($hasNet) {
    ...
    if (Test-Path ".\requirements-transcribe.txt") {
...
function Ensure-FFmpeg {
  ...
  if (-not (Test-Internet)) {
    Write-Warning "FFmpeg not found and no internet. Install FFmpeg or place ffmpeg.exe in common\bin\"
    return
  }
  ...
  # Stable Windows builds (Gyan.dev)
...
function Start-Ollama {
  ...
  if (-not $ollama) { Write-Warning "Ollama not found. Install: winget install Ollama.Ollama"; return $null }
  $dir = Split-Path $ollama -Parent
  if ($env:Path -notlike "*$dir*") { $env:Path = "$dir;$env:Path" }
  # Start if port not listening
  $listening = $false
  try { $tcp = Test-NetConnection "localhost" -Port 11434 -WarningAction SilentlyContinue; $listening = [bool]$tcp.TcpTestSucceeded } catch {}
  if (-not $listening) {
    Write-Host "Starting Ollama (serve)..." -ForegroundColor Cyan
    Start-Process -FilePath $ollama -ArgumentList "serve" -WindowStyle Hidden | Out-Null
    Start-Sleep 2
  }
  try { Invoke-RestMethod "$OllamaHost/api/tags" -TimeoutSec 3 | Out-Null; Write-Host "Ollama API: OK ($OllamaHost)" -ForegroundColor Green }
  catch { Write-Warning "Ollama API not reachable at $OllamaHost"; }
  if ($PullModel) { & $ollama pull $Model }
  return $ollama
}
if ($Portable) { $env:CITL_PORTABLE="1" }
Write-Host "== CITL Factbook + Transcription Bootstrap ==" -ForegroundColor White
Write-Host "Repo: $Repo" -ForegroundColor DarkGray
Ensure-Venv
Ensure-PythonDeps
Ensure-FFmpeg
Start-Ollama | Out-Null
# Launch GUI (prefer subfolder GUI if present)
...
if (Test-Path $gui1) {
  ...
  throw "No GUI found. Expected: factbook-assistant\factbook_assistant_gui.py OR factbook_assistant_gui.py"
}
