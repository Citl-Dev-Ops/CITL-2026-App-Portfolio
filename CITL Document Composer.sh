...
LINUX_EXE="$HERE/1-CITL-SYNC/linux/CITL Document Composer/CITL Document Composer"
if [ -x "$LINUX_EXE" ]; then
    nohup "$LINUX_EXE" &>/dev/null &
    echo "[ OK ] Launched (Linux binary): CITL Document Composer"
    exit 0
fi
...
# Find the script
for script_path in     "$HOME/Desktop/CITL/factbook-assistant/citl_doc_composer.py"     "$HOME/Documents/CITL/factbook-assistant/citl_doc_composer.py"     "$HOME/CITL/factbook-assistant/citl_doc_composer.py"     "$HERE/factbook-assistant/citl_doc_composer.py"; do
...
Please enter your password if prompted." 2>/dev/null &
        ...
        echo "[ OK ] Launched: CITL Document Composer"
        exit 0
    fi
done

echo "[ERROR] CITL Document Composer not found. Run START_CITL_UBUNTU.sh to install first."
if command -v zenity &>/dev/null; then
    zenity --error --text="CITL Document Composer not found.
