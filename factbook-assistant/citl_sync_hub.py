...
def _bash_clone_usb(src_usb: str, dst_usb: str, mode: str) -> str:
    folder_map = {
        "all":          "1-CITL-SYNC 2-CITL-PRESENTATION-SUITE 3-CITL-WORKSTATION-APPS 4-CITL-FIELD-APPS",
        "sync":         "1-CITL-SYNC",
...
def _ps_clone_usb(src_usb: str, dst_usb: str, mode: str) -> str:
    """
    mode: 'all' | 'sync' | 'workstation' | 'field' | 'presentation'
...
TILES = [
    ...
    },
    {
        ...
        "sub":   "Check all 4 USB exe bundles + local dist/",
        "color": "#2A1A00",
        "icon":  "STAT",
    },
    {
    ...
    def _run_status_check(self):
        ...
        def _work():
            ...
            if repo:
                ...
                if dist.exists():
                    for name, folder, exe in APP_BUNDLES:
                        # dist folder name = display name
                        p = dist / name / exe
                        if p.exists():
                            sz = sum(f.stat().st_size for f in (dist/name).rglob("*")
                                     if f.is_file()) // (1024*1024)
