...
$fb    = Join-Path $Repo "factbook-assistant"

if (Test-Path -LiteralPath $tools) { $targets += Get-ChildItem -Path $tools -Filter "*transcrib*.py" -File -ErrorAction SilentlyContinue }
if (Test-Path -LiteralPath $fb)    { $targets += Get-ChildItem -Path $fb    -Filter "*transcrib*.py" -File -ErrorAction SilentlyContinue }
