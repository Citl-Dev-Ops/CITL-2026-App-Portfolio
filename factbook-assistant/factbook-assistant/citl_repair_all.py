...
def run_gui(start_path: Optional[Path] = None):
    ...
    def _load_patch_log(hint_path: Optional[Path] = None):
        ...
        def _bg():
            ...
            for repo_idx, git_root in enumerate(git_repos):
                ...
                commits = _git_commits(git_root, max_count=300)
                if not commits:
    ...
    def _apply_patch_log_to_all_repos():
        if _apply_running[0]:
        ...
        sig = _current_patch_signature()
        _touch_machine("apply_start", f"signature={sig}")
        _set_status("Applying USB latest files to all found repos...")
        _plog("", "dim")
        _plog("Applying USB latest files to all found repos...", "header")
        ...
        def _bg():
            repos = _find_git_repos_on_machine()
            if not repos:
                _touch_machine("apply_no_repos", f"signature={sig}")
                _plog("No git repos found on this machine.", "err")
                root.after(0, lambda: _done("No git repos found to apply.", _T["warn"]))
                return

            _plog(f"Found {len(repos)} repo(s) to apply.", "ok")
            synced = 0
            skipped = 0
            copied_files = 0

            for idx, repo in enumerate(repos, start=1):
                _plog(f"[{idx:02d}/{len(repos):02d}] Applying to: {repo}", "batch")
                try:
                    if repo.resolve() == REPO_ROOT.resolve():
                        _plog("  SKIP source USB repo.", "dim")
                        skipped += 1
                        continue
                except OSError:
                    pass
            ...
            _plog("", "dim")
            _plog(
                f"Apply complete: {copied_files} file(s) synced across "
                f"{synced} repo(s); {skipped} skipped.",
                "ok",
            )
            _touch_machine(
                "apply_complete",
                f"signature={sig};synced={synced};skipped={skipped};files={copied_files}",
            )
            root.after(0, lambda: _done(
                f"Apply complete - {synced} repo(s) updated, {skipped} skipped.",
                _T["ok"]))
    ...
    def _capture_workstation_updates_to_usb():
        if _capture_running[0]:
            _set_status("Capture already running...", _T["warn"])
            return

        repos = _find_git_repos_on_machine()
        repos = [r for r in repos if _safe_resolve_str(r) != _safe_resolve_str(REPO_ROOT)]
        if not repos:
            _set_status("No workstation repos found for capture.", _T["warn"])
            _plog("No workstation repos found for capture.", "err")
            return
        ...
        _touch_machine("capture_start", f"repo_count={len(repos)}")
        _set_status("Preparing reverse capture from workstation repos...")
        _plog("", "dim")
        _plog("Preparing reverse capture from workstation repos...", "header")
        ...
        for repo in repos:
            ...
            _plog(f"[PLAN] {repo}  | {len(changed)} changed path(s)", "batch")
        ...
        def _bg_capture():
            ...
            for idx, plan in enumerate(plans, start=1):
                ...
                _plog(f"[{idx:02d}/{len(plans):02d}] Capture repo -> USB: {repo}", "batch")
                if not entries:
        ...
        _set_status(f"Reverse capture running ({len(plans)} repo plan(s))...")
    ...
    _apply_btn_ref[0] = _btn(
        ph2, "Apply USB Patch To All Repos", _T["warn"], _apply_patch_log_to_all_repos)
    _apply_btn_ref[0].pack(side="left", padx=(0, 4))
    _capture_btn_ref[0] = _btn(
        ph2, "Capture Workstation Updates -> USB", _T["accent"], _capture_workstation_updates_to_usb)
    _capture_btn_ref[0].pack(side="left", padx=(0, 4))
    _btn(ph2, "Add Repo...", _T["btn"], _add_repo_to_patch_cycle).pack(side="left")
...
"""
    ...
    def _auto_propagate(found: List[Path]):
        ...
        for repo in found:
            propagate_usb_bundle_to_repo(repo, log=lambda _m: None)
    ...
    def _on_quick_done(results: List[Path]):
        ...
        if results:
            ...
            # Auto-propagate repair scripts to found repos (background)
            threading.Thread(target=lambda: _auto_propagate(results),
                             daemon=True).start()
    ...
    # Also wire deep search to auto-propagate
