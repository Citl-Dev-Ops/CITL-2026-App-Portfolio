...
LINUX_EXE="$HERE/1-CITL-SYNC/linux/CITL Staff Toolkit/CITL Staff Toolkit"
if [ -x "$LINUX_EXE" ]; then
    nohup "$LINUX_EXE" &>/dev/null &
    echo "[ OK ] Launched (Linux binary): CITL Staff Toolkit"
    exit 0
fi
...
# Find the script
for script_path in     "$HOME/Desktop/CITL/factbook-assistant/citl_staff_toolkit.py"     "$HOME/Documents/CITL/factbook-assistant/citl_staff_toolkit.py"     "$HOME/CITL/factbook-assistant/citl_staff_toolkit.py"     "$HERE/factbook-assistant/citl_staff_toolkit.py"; do
...
Please enter your password if prompted." 2>/dev/null &
        ...
        echo "[ OK ] Launched: CITL Staff Toolkit"
        exit 0
    fi
done

echo "[ERROR] CITL Staff Toolkit not found. Run START_CITL_UBUNTU.sh to install first."
if command -v zenity &>/dev/null; then
    zenity --error --text="CITL Staff Toolkit not found.
